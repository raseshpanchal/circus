...
<div class="table-responsive">
    <table class="table table-striped table-bordered table-hover">
        ...
        <tbody style="color:#666" id="table">
            <?php
			$i=1;
			$query_1=mysql_query("SELECT * FROM web_images ORDER BY ID DESC");
			while($view_1=mysql_fetch_array($query_1))
			{
				$newID=$view_1['ID'];
				$newTitle=urldecode($view_1['Title']);
				$newPicture=$view_1['Picture'];
			?>
             <tr <?php echo 'id="td'.$newID.'"'; ?>>
                <td id="1_<?php echo $newID ?>" width="40" align="center"><?php echo $i; ?></td>
                <td id="2_<?php echo $newID ?>" width="40">
				<?php echo $newTitle; ?></td>
                 <td id="3_<?php echo $newID ?>">
                    http://www.ambizgroup.in/web-img/<?php echo $newPicture; ?>
                </td>
                <td width="40" align="center">
                    <a href="app/viewWebImage?ID=<?php echo $newID; ?>" class="btnView">
                        <img src="images/doc.png" border="0" />
                    </a>
                </td>
                <td width="40" align="center">
                    <a href="app/deleteWebImage?ID=<?php echo $newID; ?>" class="btnDelete">
...
function dataExport()
{
	window.open("exportResources", "myWindow", "scrollbars=no, toolbar=no, menubar=no, status=no, width=550, height=325, resizable=0");
}
...
$(document).ready(function(){
	...
	$("#btnRefresh").click(function(){
		...
		closeForm();
		$("#middleArea").load("view/webImages");
	});

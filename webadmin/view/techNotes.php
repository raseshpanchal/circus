...
<div class="table-responsive">
    <table class="table table-striped table-bordered table-hover">
        ...
        <tbody style="color:#666" id="table">
            <?php
			$i=1;
			$query_1=mysql_query("SELECT * FROM technical_notes ORDER BY ID DESC");
			while($view_1=mysql_fetch_array($query_1))
			{
				$newID=$view_1['ID'];
				$newNotes=urldecode($view_1['Notes']);
            ?>
             <tr <?php echo 'id="td'.$newID.'"'; ?>>
                <td id="1_<?php echo $newID ?>" width="40" align="center"><?php echo $i; ?></td>
                <td id="2_<?php echo $newID ?>">
				<?php echo $newNotes; ?></td>
                <td width="40" align="center">
                    <a href="app/editTechNotes?ID=<?php echo $newID; ?>" class="btnEdit">
...
$(document).ready(function(){
	...
	$("#btnRefresh").click(function(){
		...
		closeForm();
		$("#middleArea").load("view/techNotes");
	});

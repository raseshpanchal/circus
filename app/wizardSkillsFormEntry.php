...
$newSubCat=myDecode($_POST['txt_subCat']);
...
$query_2=mysqli_query($link, "UPDATE freelancer_registration SET Status='Active' WHERE ID='$userID' ");
...
if(isset($_POST['checkboxvar']))
{
    ...
    foreach($chpmods as $chpmod)
    {
        ...
        $query_4=mysqli_query($link, "SELECT * FROM freelancer_skills WHERE FreelancerID='$userID' AND SkillID='$chpmod'");
        $product_rows=mysqli_num_rows($query_2);
        if($product_rows==0)
        {
            $query_5=mysqli_query($link, "INSERT INTO freelancer_skills SET FreelancerID='$userID', SkillID='$chpmod', SkillName='$skillName'");
        }
    }
}
